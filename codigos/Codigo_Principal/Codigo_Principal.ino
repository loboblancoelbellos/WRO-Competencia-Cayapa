// =========================================================================
// DINOCO 2.0 - CÓDIGO PRINCIPAL DE NAVEGACIÓN AUTÓNOMA
// Equipo Cayapa - WRO 2026 Future Engineers (Open Challenge)
// =========================================================================
// Hardware:
//   - 1x ESP32 (30 pines)
//   - 3x Sensor ultrasónico HC-SR04 (izquierdo, central, derecho)
//   - 1x Puente H L298N (motor de tracción trasero)
//   - 1x Servomotor MG90S (dirección Ackermann delantera)
//
// Pinout (ver tabla completa en README.md, sección 7.3):
//   Ultrasónico Izquierdo  -> TRIG 15 | ECHO 2
//   Ultrasónico Central    -> TRIG 27 | ECHO 14
//   Ultrasónico Derecho    -> TRIG 13 | ECHO 12
//   Puente H L298N         -> ENA 25  | IN1 33 | IN2 32
//   Servomotor MG90S       -> Señal 26
//
// Estrategia:
//   1. Seguimiento de carril mediante PD mientras el auto avanza en recta,
//      comparando la distancia lateral izquierda vs derecha.
//   2. Detección de esquina abierta (hueco en un lateral) o pared frontal
//      cercana para decidir el sentido de giro (horario/antihorario).
//   3. Una vez detectado el primer giro, el sentido de la pista queda fijo
//      para el resto del recorrido (regla oficial WRO: la pista gira
//      siempre en el mismo sentido).
//   4. Conteo de vueltas: la pista tiene 4 esquinas por vuelta. Al
//      completar 3 vueltas (12 esquinas) el auto se detiene con las
//      ruedas centradas, cumpliendo el reto abierto.
// =========================================================================
...
// ==========================================
// INTELIGENCIA DE INTERSECCIONES Y VUELTAS
// ==========================================
enum SentidoPista { DESCONOCIDO, HORARIO, ANTIHORARIO };
SentidoPista sentido = DESCONOCIDO;

enum EstadoVehiculo { NAV_RECTA, GIRO_ESQUINA, SALIDA_CURVA, DETENIDO };
EstadoVehiculo estadoActual = NAV_RECTA;

unsigned long tiempoInicioEstado = 0;
int anguloGiroActual = CENTRO_PERFECTO;

// Reglamento: 4 esquinas por vuelta x 3 vueltas obligatorias
const int ESQUINAS_POR_VUELTA = 4;
const int VUELTAS_OBJETIVO = 3;
const int ESQUINAS_OBJETIVO = ESQUINAS_POR_VUELTA * VUELTAS_OBJETIVO;
int esquinasCompletadas = 0;
...
void loop() {
  if (estadoActual == DETENIDO) {
    return; // Recorrido finalizado: no se ejecuta más lógica de navegación
  }
  ...
  switch (estadoActual) {
    ...
    case NAV_RECTA: {
      ...
      if (tiempoActual - tiempoAnterior >= INTERVALO_COMPENSACION) {
        ...
        if (distIzq < ESCUDO_LATERAL) {
          direccionServo.write(constrain(CENTRO_PERFECTO - FUERZA_ESCUDO, 0, 180));
          ultimoError = 0;
        }
          ...
          direccionServo.write(constrain(CENTRO_PERFECTO + correccion, 0, 180));
        }
      }
      break;
    }
    ...
    case SALIDA_CURVA: {
      ...
      if (tiempoEnSalida < 150) {
        ...
        if (anguloGiroActual == GIRO_IZQ_ABIERTO) {
          direccionServo.write(constrain(CENTRO_PERFECTO - IMPULSO_COMPENSACION, 0, 180)); // Tuerce derecha
        } else {
          direccionServo.write(constrain(CENTRO_PERFECTO + IMPULSO_COMPENSACION, 0, 180)); // Tuerce izquierda
        }
      }
    ...
    case DETENIDO:
      // Manejado al inicio de loop()
      break;
  }
}

// ==========================================
// Marca una esquina como completada, cuenta las vueltas y decide
// si el recorrido terminó (3 vueltas reglamentarias) o continúa.
// ==========================================
void registrarEsquinaCompletada() {
  esquinasCompletadas++;
  Serial.print("Esquina completada: ");
  Serial.print(esquinasCompletadas);
  Serial.print(" / ");
  Serial.println(ESQUINAS_OBJETIVO);

  if (esquinasCompletadas >= ESQUINAS_OBJETIVO) {
    estadoActual = DETENIDO;
    direccionServo.write(CENTRO_PERFECTO);
    detenerMotores();
    Serial.println("RECORRIDO FINALIZADO: 3 vueltas completadas. Auto detenido.");
  } else {
